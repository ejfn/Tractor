...
1. **System prompt — `STATIC_LLM_GAME_RULES`**: the static, game-wide strategic rules (§1 Objective, §2 Card Strength, §3 Combos & Tractors, §4 Following — Absolute Laws, §5 Reading Options & Strategy, §6 Leading — Strategy Order). This is the bot's standing "how to play" knowledge.
2. **User prompt — `buildUserPromptTemplate`**: the per-decision template, filled in by `buildLLMUserPrompt` and its helpers in [src/ai/llm/llmGamePrompt.ts](file:src/ai/llm/llmGamePrompt.ts) with the dynamic context for THIS decision:
   - **Current State**: Player role (attacking/defending), attacking team points, score pressure, round progress, trump rank/suit, declarer ID, and live unseen off-suit points.
   - **Recent Tricks**: Recent trick history.
   - **Confirmed Voids**: Confirmed player suit voids.
   - **Active Trick Status** (when following): Led player, led group, requirement, plays so far, seat position, currently winning player (teammate/opponent), and trick points.
   - **Option Consequences**: Candidate lead/follow options generated by [src/ai/llm/llmPositionDiagnosis.ts](file:src/ai/llm/llmPositionDiagnosis.ts) (`buildLeadingOptions` when leading, `buildFollowingOptions` when following).
...
- **Dynamic gap** (the bot lacks, or misreads, a per-decision signal) → improve what `buildUserPromptTemplate` / `llmGamePrompt.ts` surface, how `llmPositionDiagnosis.ts` frames option consequences, or how the static rules tell the bot to interpret those signals.

### The neutral option diagnosis layer

Candidate options in **## Lead Options** / **## Your Options** are generated by `llmPositionDiagnosis.ts`. Following the neutrality refactor (`b652a0e`), options present factual consequences neutrally — trick win/loss, points captured/fed, card cost/scarcity — without pre-baked recommendations or artificial rule scores.

For example, leading an unbeatable trump single states: `wins the trick; opponents follow with their lowest trump so it captures ≈0 pts; this trump is gone for later ruffs and the final trick`. This presents the full factual trade-off (guaranteed win vs asset consumption) so the LLM can make an unanchored strategic judgement based on current score pressure and round progress.
