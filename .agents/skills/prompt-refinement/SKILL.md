...
For example, leading an unbeatable trump single states: `captures ≈0 pts as opponents follow low; burns your top trump (lost for ruffing points and final-trick multiplier)`. This presents the full factual trade-off (guaranteed win vs asset consumption) so the LLM can make an unanchored strategic judgement based on current score pressure and round progress.
