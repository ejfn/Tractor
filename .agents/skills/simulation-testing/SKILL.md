...
Both the simulation test (`__tests__/simulation/gameSimulation.test.ts`) and the runner script (`.agents/skills/simulation-testing/scripts/run-simulation.sh`) support a robust environment-loading hierarchy to manage API keys and configurations securely:
