...
export async function callLLMForDecision(
  ...
  // 1. Generate verified Strategic Lines
  const strategicLines: StrategicLine[] = isLeading
    ? generateLeadingLines(
        gameState,
        hand,
        playerId,
  ...
  // If 0 or 1 line available, no ambiguous choice
  if (strategicLines.length === 0) {
    recordLLMDuration(Date.now() - decisionStartTime);
    return fallback;
  }
  if (strategicLines.length === 1) {
    recordLLMDuration(Date.now() - decisionStartTime);
    return strategicLines[0].cards;
  }

  // 2. Retrieve persistent strategy plan
  const currentPlan = getActiveRoundPlan(
    playerId,
    gameContext.isAttackingTeam,
    gameState.tricks.length,
  );

  // 3. Build ultra-compact prompt (<400 tokens)
    ...
    if (selectedLine) {
      ...
      if (parsedResponse?.strategy) {
        updateActiveRoundPlan(
          playerId,
          {
            goal: parsedResponse.strategy as RoundStrategyGoal,
            reasoning: parsedResponse.reason ?? currentPlan.reasoning,
          },
          gameState.tricks.length,
        );
      }
      ...
      gameLogger.info("llm_decision_success", {
        ...
        intent: selectedLine.intent,
        reasoning: parsedResponse?.reason ?? "Selected line " + selectedLine.id,
        play: selectedLine.cards.map((c) => c.toString()),
      });
      ...
      llmSuccessfulPlays++;
      recordLLMDuration(Date.now() - decisionStartTime);
      return selectedLine.cards;
    } else {
      // Parsing could not match a line ID, fall back to top strategic line
      gameLogger.warn("llm_line_parse_fallback", {
        playerId,
        rawResponse: responseText,
        availableLines: strategicLines.map((l) => l.id),
      });
