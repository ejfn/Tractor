import { Card, GameState, PlayerId, Rank, TrickPosition } from "../../types";
...
export async function selectLLMPlayAsync(
  ...
  if (isLeading) {
    const candidates = detectCandidateLeads(player.hand, gameState, playerId, trumpInfo);
    if (candidates.length === 0) {
      ...
      const bestNonTrump = nonTrumpCandidates.length > 0 ? nonTrumpCandidates[0] : null;
      const bestTrump = trumpCandidates.length > 0 ? trumpCandidates[0] : null;
      const bestOverall = (bestNonTrump && bestTrump)
        ? (bestNonTrump.result.score >= bestTrump.result.score ? bestNonTrump : bestTrump)
      ...
      if (bestOverall) {
        ...
        if (isRoundStart && isHighAce && !bestOverall.candidate.metadata.isTrump) {
          isClearDecision = true;
        ...
        // Ambiguous Lead: engage LLM with candidates list
          ...
          engagementContext = {
            dilemma: "We need to choose which suit or combination to lead. We want to secure the lead, pressure opponents, avoid leading under opponents' voids, and avoid wasting point cards prematurely.",
    ...
    const analysis = analyzeSuitAvailability(leadingCards, player.hand, trumpInfo);
    ...
    else if (analysis.remainingCards && analysis.remainingCards.length === analysis.requiredLength) {
      isClearDecision = true;
    ...
    else if (analysis.scenario === "valid_combos" && analysis.validCombos && analysis.validCombos.length === 1) {
      isClearDecision = true;
    ...
    // Ambiguous Following: engage LLM based on scenarios
      ...
      if (analysis.scenario === "void" && player.hand.some(c => isTrump(c, trumpInfo))) {
      ...
      } else if (analysis.scenario === "insufficient" || (analysis.scenario === "void" && !player.hand.some(c => isTrump(c, trumpInfo)))) {
  ...
  while (attempt < maxAttempts) {
    ...
    try {
      ...
      const prompt = buildLLMUserPrompt(gameState, playerId, player.hand, engagementContext);
      ...
      try {
        ...
        errorHint = "Failed to parse your response as JSON. Please ensure your play selection strictly follows JSON formatting. Example: { \"reasoning\": \"explanation\", \"play\": [\"c1\"] }";
      ...
      if (!Array.isArray(parsedPlay) || parsedPlay.length === 0) {
        ...
        errorHint = "Your response did not contain a valid 'play' array of card IDs (e.g. ['c1', 'c2']). Please select valid card IDs from your hand.";
        llmInvalidCardRetries++;
        continue;
      }
      ...
      if (uniquePlayIds.size !== parsedPlay.length) {
        ...
        errorHint = "Your selection contains duplicate card choice IDs (e.g. selecting the same ID 'c1' twice). Each card ID in your play selection must be unique. Please try again with unique card IDs.";
        llmInvalidCardRetries++;
        continue;
      }
