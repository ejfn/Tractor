...
export function analyzeTrickWinner(
  ...
  const currentWinner = currentTrick.winningPlayerId;
  ...
  const isTeammateWinning = !isSelfWinning && isTeammate(gameState, playerId, currentWinner);
 ...
 */
function isTeammate(gameState: GameState, playerId1: string, playerId2: string): boolean {
...
function canPlayerBeatCurrentWinner(
  ...
  if (currentTrick.winningPlayerId === currentTrick.leadingPlayerId) {
    currentWinnerCards = currentTrick.leadingCombo;
  } else {
    const winningPlay = currentTrick.plays.find(
      (play: { playerId: string; cards: Card[] }) => play.playerId === currentTrick.winningPlayerId
    );
  ...
  const followingCards = player.hand.filter(card => card.suit === leadingSuit);
  ...
  if (!currentWinnerHasTrump) {
    ...
    const hasTrump = player.hand.some(card => isTrump(card, gameState.trumpInfo));
    if (hasTrump) return true;
    
    // Check if we have higher same-suit cards
    const winnerCard = currentWinnerCards[0];
    const hasHigherCard = followingCards.some(card => 
      compareCards(card, winnerCard, gameState.trumpInfo) > 0
    );
...
function determineIfShouldPlayConservatively(
  ...
  // Play conservatively if teammate is winning
  if (isTeammateWinning) return true;
