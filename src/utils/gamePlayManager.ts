...
export function processPlay(
  ...
  const isTrickComplete =
    newState.currentTrick && newState.currentTrick.plays.length === 3; // 4 players - 1 leader

  // Check if this player is the winner of the previous trick
  const isWinner =
    newState.tricks.length > 0 &&
    currentPlayerId ===
      newState.tricks[newState.tricks.length - 1].winningPlayerId;
  ...
  const playPoints = cards.reduce((sum, card) => sum + (card?.points || 0), 0);
  ...
  newState.players[currentPlayerId].hand = newHand;
  ...
  if (newState.currentTrick.plays.length === 3) {
    // 4 players - 1 leader
    // Find the winner
    ...
    const trickWinningPlayer = newState.players[winningPlayerId];
    if (trickWinningPlayer) {
      const winningTeam = newState.teams[trickWinningPlayer.teamId];
      if (winningTeam) {
    ...
    const winningPlayer = newState.players[winningPlayerId];
 ...
 */
export function getAIMoveWithErrorHandling(
  state: GameState,
  currentPlayerId: string,
): {
  cards: Card[];
  ...
  try {
    ...
    if (!currentPlayer) {
      return {
        cards: [],
        error: `Player with ID ${currentPlayerId} not found`,
      };
    }
