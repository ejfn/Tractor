...
class AIStrategy implements AIStrategy {
  makePlay(
    ...
    const allPlayers = GameStateUtils.getPlayersInOrder(gameState);
    ...
    const partnerPlayer = allPlayers[partnerIndex];
    const partnerInTrick = currentTrick.plays.some(
      (play) => play.playerId === partnerPlayer.id,
    );

    const currentWinner = this.getCurrentTrickWinner(gameState);
    const partnerWinning = partnerInTrick && currentWinner === partnerPlayer.id;
...
export const getAIMove = (gameState: GameState, playerId: string): Card[] => {
  ...
  const allPlayers = GameStateUtils.getPlayersInOrder(gameState);
  const currentPlayerIndex = allPlayers.findIndex((p) => p.id === player.id);
