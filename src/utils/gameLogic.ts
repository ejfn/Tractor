...
export const dealCards = (state: GameState): GameState => {
  ...
  const cardsPerPlayer = Math.floor((deck.length - 8) / playerIds.length);

  playerIds.forEach((playerId, index) => {
    const startIdx = index * cardsPerPlayer;
    newState.players[playerId].hand = deck.slice(
      startIdx,
...
export const initializeGame = (
  ...
  // Create teams as Record
  const teams: Record<"A" | "B", Team> = {
    A: {
      id: "A",
  ...
  const playerData = [
    { id: "player", name: playerName, isHuman: true, teamId: "A" as const },
    { id: "ai1", name: "Bot 1", isHuman: false, teamId: "B" as const },
    { id: "ai2", name: "Bot 2", isHuman: false, teamId: "A" as const },
    { id: "ai3", name: "Bot 3", isHuman: false, teamId: "B" as const },
  ];
  ...
  // Deal cards to players (update their hands in playerStates)
