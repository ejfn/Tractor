...
export function prepareNextRound(
  ...
  const newDefendingTeam = Object.values(newState.teams).find(
    (t) => t.isDefending,
  );
  if (newDefendingTeam) {
  ...
  // Deal cards
  let cardIndex = 0;
  const cardsPerPlayer = Math.floor((deck.length - 8) / 4); // Always 4 players
  ...
  // Note: Round starting player is determined by PlayerStateManager
  // based on game rules:
  // - First round: Trump declarer goes first
  // - Subsequent rounds: Based on previous round outcome and team rotation

  // Player order tracking is now handled by PlayerStateManager
...
export function endRound(state: GameState): {
  ...
  const defendingTeam = Object.values(newState.teams).find(
    (t) => t.isDefending,
  );
  const attackingTeam = Object.values(newState.teams).find(
    (t) => !t.isDefending,
  );
