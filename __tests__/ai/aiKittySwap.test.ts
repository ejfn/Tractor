...
function createKittySwapGameState(): GameState {
  ...
  let cardIndex = 0;
  for (let i = 0; i < 23; i++) {
    // 23 non-trump cards to reach 33 total
    const suit = nonTrumpSuits[cardIndex % nonTrumpSuits.length];
    const rank =
      nonTrumpRanks[
        Math.floor(cardIndex / nonTrumpSuits.length) % nonTrumpRanks.length
      ];
...
describe("AI Kitty Swap Strategy", () => {
  ...
  describe("Core Requirements", () => {
    ...
    test("should usually avoid trump cards, but allow when hand is exceptionally strong", () => {
      ...
      const trumpCardsInKitty = selectedCards.filter((card) =>
      ...
      if (trumpCardsInKitty.length > 0) {
        ...
        const player = getPlayerById(gameState, botPlayerId);
        const trumpCards = player.hand.filter((card) =>
          isTrump(card, gameState.trumpInfo),
        );
        const nonTrumpCards = player.hand.filter(
          (card) => !isTrump(card, gameState.trumpInfo),
        );
    ...
    test("should only select cards from player's hand", () => {
      ...
      selectedCards.forEach((card) => {
        expect(player.hand.some((handCard) => handCard.id === card.id)).toBe(
          true,
        );
      });
    });
  ...
  describe("Simple Strategic Tests", () => {
    test("should prefer non-point cards when available", () => {
      ...
      for (let i = 0; i < 8; i++) {
        simpleNonPointCards.push(
          Card.createCard(Suit.Hearts, Rank.Three, (i % 2) as DeckId),
        );
      }
    ...
    test("should strategically include trump cards when hand is exceptionally strong", () => {
      ...
      for (let i = 0; i < 18; i++) {
        longTrumpHand.push(
          Card.createCard(Suit.Spades, Rank.Three, (i % 2) as DeckId),
        );
      }
      ...
      for (let i = 0; i < 7; i++) {
        longTrumpHand.push(
          Card.createCard(Suit.Hearts, Rank.Four, (i % 2) as DeckId),
        );
      }
      ...
      for (let i = 18; i < 26; i++) {
        longTrumpHand.push(
          Card.createCard(Suit.Spades, Rank.Four, (i % 2) as DeckId),
        );
      }
      ...
      const nonTrumpCardsInKitty = selectedCards.filter(
        (card) => !isTrump(card, gameState.trumpInfo),
      );
    ...
    test("should execute strategic suit elimination when beneficial", () => {
      ...
      for (let i = 0; i < 10; i++) {
        ...
        strategicHand.push(
          Card.createCard(suit, Rank.Nine, ((i + 1) % 2) as DeckId),
        );
      }
      ...
      const heartsAceInKitty = selectedCards.some(
        (card) => card.rank === "A" && card.suit === "Hearts",
      );
      const heartsKingInKitty = selectedCards.some(
        (card) => card.rank === "K" && card.suit === "Hearts",
      );
      ...
      const nonTrumpInKitty = selectedCards.filter(
        (card) => !isTrump(card, gameState.trumpInfo),
      );
  ...
  describe("Trump Hierarchy Logic", () => {
    test("should prefer weak trump suit cards over strong trump rank cards when forced to dispose trump", () => {
      ...
      const trumpRankHeart = player.hand.find(
        (c) => c.rank === Rank.Two && c.suit === Suit.Hearts,
      );
      const trumpRankSpade = player.hand.find(
        (c) => c.rank === Rank.Two && c.suit === Suit.Spades,
      );
      const trumpAce = player.hand.find(
        (c) => c.rank === Rank.Ace && c.suit === Suit.Spades,
      );
      const trumpKing = player.hand.find(
        (c) => c.rank === Rank.King && c.suit === Suit.Spades,
      );
    ...
    test("should use ComboStrength analysis for disposal prioritization", () => {
      ...
      for (let i = 0; i < 25; i++) {
        mixedStrengthHand.push(
          Card.createCard(Suit.Diamonds, Rank.Seven, (i % 2) as DeckId),
        ); // Weak
      }
      ...
      const weakCardsInKitty = selectedCards.filter(
        (card) => card.rank === Rank.Seven && card.suit === Suit.Diamonds,
      );
      ...
      const criticalTrump = player.hand.find(
        (c) => c.rank === Rank.Two && c.suit === Suit.Hearts,
      );
      const strongAce = player.hand.find(
        (c) => c.rank === Rank.Ace && c.suit === Suit.Hearts,
      );
      const strongKing = player.hand.find(
    ...
    test("should factor trump conservation values into suit elimination decisions", () => {
      ...
      const clubsCardsInKitty = selectedCards.filter(
        (card) => card.suit === Suit.Clubs,
      );
      ...
      const trumpAce = suitEliminationHand.find(
        (c) => c.rank === Rank.Ace && c.suit === Suit.Spades,
      );
      const trumpKing = suitEliminationHand.find(
        (c) => c.rank === Rank.King && c.suit === Suit.Spades,
      );
