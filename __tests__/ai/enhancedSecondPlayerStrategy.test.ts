...
describe("Enhanced Second Player Strategy", () => {
  ...
  describe("Same-Suit Following - Teammate Led", () => {
    it("should contribute high cards when teammate (Bot3) leads", () => {
      const gameState = initializeGame();
      gameState.trumpInfo = trumpInfo;

      // Bot3 (teammate of Bot1) leads Q♠
      const teammateLeadingCard = Card.createCard(Suit.Spades, Rank.Queen, 0);
      ...
      // Set current player to Bot1 (2nd player)
      gameState.currentPlayerIndex = 1;

      // Bot1 hand: has Spades including point cards
      const bot1Hand = [
        Card.createCard(Suit.Spades, Rank.King, 0), // High card (10 points)
        Card.createCard(Suit.Spades, Rank.Jack, 0), // Medium card
        Card.createCard(Suit.Hearts, Rank.Ace, 0), // Trump card
        Card.createCard(Suit.Clubs, Rank.Ten, 0), // Point card other suit
      ];
      ...
      // Should contribute K♠ (highest available) to support teammate
      expect(aiDecision).toHaveLength(1);
      expect(aiDecision[0].suit).toBe(Suit.Spades);
      expect(aiDecision[0].rank).toBe(Rank.King);
    });

    it("should prioritize point cards when teammate leads", () => {
      const gameState = initializeGame();
      gameState.trumpInfo = trumpInfo;

      // Bot3 (teammate) leads 8♠
      const teammateLeadingCard = Card.createCard(Suit.Spades, Rank.Eight, 0);
      ...
      // Bot1 hand: has point cards in Spades
      const bot1Hand = [
        Card.createCard(Suit.Spades, Rank.Ten, 0), // Point card (10 points)
        Card.createCard(Suit.Spades, Rank.King, 0), // Point card (10 points)
        Card.createCard(Suit.Spades, Rank.Nine, 0), // Non-point card
      ...
      // Should play a point card (Ten or King) to contribute points
      expect(aiDecision).toHaveLength(1);
      expect(aiDecision[0].suit).toBe(Suit.Spades);
      expect([Rank.Ten, Rank.King]).toContain(aiDecision[0].rank);
    });
  });
