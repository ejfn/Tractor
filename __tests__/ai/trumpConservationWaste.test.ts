...
describe("Trump Conservation - Avoid Wasting Big Trump", () => {
  describe("Following Trump Pairs When Can't Form Pairs", () => {
    it("should use weak trump suit cards instead of jokers when following trump pairs", () => {
      ...
      gameLogger.info(
        "test_trump_conservation_selected",
        {
    ...
    it("should use trump conservation hierarchy when following trump and can't form proper combinations", () => {
      ...
      const trumpCardsUsed = selectedCards.filter(
        (card) =>
          card.joker || card.rank === Rank.Two || card.suit === Suit.Hearts,
      );
      ...
      const bigJokerUsed = selectedCards.some(
        (card) => card.joker === JokerType.Big,
      );
      const trumpRankUsed = selectedCards.some(
        (card) => card.rank === Rank.Two && card.suit === Suit.Clubs,
      );
      const trumpSuitUsed = selectedCards.some(
