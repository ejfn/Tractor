...
describe("Bot 3 Trump Following Decision Tree Bug", () => {
  test("Bot 3 should play weakest trump when opponent (Bot 2) is winning trump trick", () => {
    ...
    gameState.players[1].hand = [Card.createCard(Suit.Diamonds, Rank.Queen, 0)];

    // Bot 2: Has 2♥ (trump rank off-suit - will win the trick)
    gameState.players[2].hand = [Card.createCard(Suit.Hearts, Rank.Two, 0)];
    ...
    gameLogger.info(
      "test_bot3_trump_selection",
      { selectedCard: `${selectedCard.rank}${selectedCard.suit}` },
      `Bot 3 selected: ${selectedCard.rank}${selectedCard.suit}`,
    );
    gameLogger.info(
      "test_available_trump_options",
      { availableOptions: bot3Hand.map((c) => `${c.rank}${c.suit}`) },
      "Available trump options were: " +
        bot3Hand.map((c) => `${c.rank}${c.suit}`).join(", "),
    );
