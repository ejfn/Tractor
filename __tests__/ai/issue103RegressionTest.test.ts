...
describe("Issue #103 Regression Test - AI Wasting Valuable Trump Rank Cards", () => {
  it("should reproduce Issue #103: AI plays rank card when it cannot beat the winning card", () => {
    ...
    gameLogger.info(
      ...
      {
        keyPoint:
          "AI rank cards (2♥, 2♣) CANNOT beat Big Joker - should not waste them",
      },
    ...
    const isPlayingWeakTrump =
      selectedCard.rank === Rank.Three && selectedCard.suit === Suit.Spades;

    if (isPlayingRankCard) {
      gameLogger.error(
        "issue103_bug_reproduced",
        {
      ...
      fail(
        `Issue #103 reproduced: AI played rank card ${selectedCard.rank}${selectedCard.suit} instead of weak trump 3♠`,
      );
    } else if (isPlayingWeakTrump) {
      gameLogger.info(
        "issue103_correct_behavior",
        {
  ...
  it("should reproduce Issue #103 with different trump suits to verify bug pattern", () => {
    ...
    if (isWastingRankCard) {
      fail(
        `Issue #103 variant: AI wasted rank card ${selectedCard.rank}${selectedCard.suit} instead of weak trump 4♥`,
      );
    }
