...
describe("AI Trump Following Behavior", () => {
  it("should show AI correctly choosing trump singles when trump pairs are led", () => {
    ...
    gameState.currentTrick = {
      plays: [{ playerId: PlayerId.Human, cards: leadingTrumpPair }],
    ...
    // Check what AI chose
    const aiChoseTrumpSingles = aiMove.every(
      (card) => card.suit === Suit.Spades,
    );
    const aiChoseNonTrumpPair =
      aiMove.length === 2 &&
      aiMove.every(
  ...
  it("should handle trump pairs when AI has sufficient trump cards", () => {
    ...
    const isAIMoveValid = isValidPlay(aiMove, aiHand, PlayerId.Bot1, gameState);

    expect(isAIMoveValid).toBe(true);

    // AI should NOT play the non-trump pair
    const playedNonTrumpPair =
      aiMove.length === 2 &&
      aiMove.every(
        (card) => card.suit === Suit.Spades && card.rank === Rank.Six,
      );
  ...
  it("should use all available trump pairs when following trump tractor and cannot form tractor", () => {
    ...
    const used3Hearts = aiMove.filter(
      (card) => card.rank === Rank.Three && card.suit === Suit.Hearts,
    ).length;
    const used5Hearts = aiMove.filter(
      (card) => card.rank === Rank.Five && card.suit === Suit.Hearts,
    ).length;

    // Count complete pairs used (each pair = 2 cards)
    const completePairsUsed =
      Math.floor(used3Hearts / 2) + Math.floor(used5Hearts / 2);
  ...
  it("should use all trump pairs when following trump tractor that requires 6 cards", () => {
    ...
    const used3Spades = aiMove.filter(
      (card) => card.rank === Rank.Three && card.suit === Suit.Spades,
    ).length;
    const used7Spades = aiMove.filter(
      (card) => card.rank === Rank.Seven && card.suit === Suit.Spades,
    ).length;
    const usedAceHearts = aiMove.filter(
  ...
  it("should prioritize trump pairs in conservation order when mixed types available", () => {
    ...
    const used3Diamonds = aiMove.filter(
      (card) => card.rank === Rank.Three && card.suit === Suit.Diamonds,
    ).length;
    const used4Diamonds = aiMove.filter(
      (card) => card.rank === Rank.Four && card.suit === Suit.Diamonds,
    ).length;
    const used2Hearts = aiMove.filter(
      (card) => card.rank === Rank.Two && card.suit === Suit.Hearts,
    ).length;
    const usedBigJokers = aiMove.filter(
  ...
  it("should use all trump pairs before any trump singles when insufficient pairs available", () => {
    ...
    const used4Spades = aiMove.filter(
      (card) => card.rank === Rank.Four && card.suit === Suit.Spades,
    ).length;
    const used5Spades = aiMove.filter(
      (card) => card.rank === Rank.Five && card.suit === Suit.Spades,
    ).length;
    const used6Spades = aiMove.filter(
      (card) => card.rank === Rank.Six && card.suit === Suit.Spades,
    ).length;
    const used7Spades = aiMove.filter(
      (card) => card.rank === Rank.Seven && card.suit === Suit.Spades,
    ).length;
    const usedKingHearts = aiMove.filter(
