...
describe("Smart Teammate Void Strategy", () => {
  ...
  describe("Teammate Void Point Collection", () => {
    it("should lead teammate void when opponents have many points", () => {
      ...
      const teammateVoidOpportunity = voidAnalysis.exploitableVoids.find(
        (opp) =>
          opp.targetPlayer === PlayerId.Bot2 && opp.voidSuit === Suit.Hearts,
      );

      expect(teammateVoidOpportunity).toBeDefined();
      expect(teammateVoidOpportunity?.exploitationType).toMatch(
        /point_collection|setup_teammate/,
      );
      expect(teammateVoidOpportunity?.expectedOutcome).toContain(
        "points for team",
      );
    ...
    it("should generate void-based lead for teammate setup", () => {
      ...
      expect(voidAnalysis.voidBasedLeadRecommendations.length).toBeGreaterThan(
        0,
      );

      const teammateSetupLead = voidAnalysis.voidBasedLeadRecommendations.find(
        (lead) =>
          lead.strategicGoal === "setup_teammate" ||
          lead.strategicGoal === "collect_points",
      );
      ...
      expect(teammateSetupLead?.expectedResponse[PlayerId.Bot2]).toContain(
        "Trump",
      );
    });
    ...
    it("should avoid leading teammate void when few points available", () => {
      ...
      const teammateVoidOpportunity = voidAnalysis.exploitableVoids.find(
        (opp) =>
          opp.targetPlayer === PlayerId.Bot2 && opp.voidSuit === Suit.Diamonds,
      );
  ...
  describe("Opponent vs Teammate Void Differentiation", () => {
    ...
    it("should handle mixed void scenarios correctly", () => {
      ...
      expect(teammateVoid?.exploitationType).toMatch(
        /point_collection|setup_teammate/,
      );
  ...
  describe("Strategic Decision Making", () => {
    it("should recommend optimal void exploitation sequence", () => {
      ...
      expect(
        voidAnalysis.voidTimingRecommendations.immediateOpportunities.length,
      ).toBeGreaterThan(0);
