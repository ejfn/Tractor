...
describe("Non-Trump Pair Waste When Following Trump Pair Bug", () => {
  ...
  it("should NOT waste non-trump pairs when following trump pair lead", () => {
    ...
    gameLogger.info(
      "test_non_trump_pair_waste_setup",
      {
    ...
    gameLogger.info(
      "test_lead_cards",
      { lead: "7♠-7♠" },
      "- Lead: 7♠-7♠ (trump suit pair)",
    );
    gameLogger.info("test_bot_hand", { player: "Bot1" }, "- Bot1 has:");
    gameLogger.info(
      "test_trump_pairs",
      { pairs: ["3♠-3♠", "2♣-2♥"] },
      "  * Trump pairs: 3♠-3♠, 2♣-2♥ (SHOULD use these)",
    );
    gameLogger.info(
      "test_non_trump_pairs",
      { pairs: ["K♥-K♥", "Q♣-Q♣"] },
      "  * Non-trump pairs: K♥-K♥, Q♣-Q♣ (should NOT use these)",
    );
    gameLogger.info(
      "test_expectation",
      { expected: "use trump pair, not non-trump pair" },
      "Expected: Bot1 should use trump pair, NOT non-trump pair",
    );
    ...
    const usedNonTrump = aiMove.some(
      (card) =>
        !usedTrump &&
  ...
  it("should use trump rank pairs when following trump rank pair lead", () => {
    ...
    gameLogger.info(
      "test_lead_cards",
      { lead: "2♥-2♣" },
      "- Lead: 2♥-2♣ (trump rank pair)",
    );
    gameLogger.info(
      "test_trump_rank_pair",
      { pair: "2♦-2♠" },
      "- Bot1 has trump rank pair: 2♦-2♠",
    );
    gameLogger.info(
      "test_trump_suit_pair",
      { pair: "5♠-5♠" },
      "- Bot1 has trump suit pair: 5♠-5♠",
    );
    gameLogger.info(
      "test_non_trump_pair",
      { pair: "A♥-A♥" },
      "- Bot1 has non-trump pair: A♥-A♥",
    );
    gameLogger.info(
      "test_expectation",
      { expected: "use trump pair (either trump rank or trump suit)" },
      "Expected: Bot1 should use trump pair (either trump rank or trump suit)",
    );
    ...
    const usedNonTrumpAces = aiMove.some(
      (card) => card.suit === Suit.Hearts && card.rank === Rank.Ace,
    );
  ...
  it("should use weakest trump pairs when following trump, preserving stronger trump", () => {
    ...
    gameLogger.info(
      "test_lead_cards",
      { lead: "4♠-4♠" },
      "- Lead: 4♠-4♠ (trump suit pair)",
    );
    gameLogger.info(
      "test_weak_trump",
      { pair: "3♠-3♠" },
      "- Bot1 has weak trump: 3♠-3♠",
    );
    gameLogger.info(
      "test_strong_trump",
      { pair: "A♠-A♠" },
      "- Bot1 has strong trump: A♠-A♠",
    );
    gameLogger.info(
      "test_trump_rank",
      { pair: "2♥-2♣" },
      "- Bot1 has trump rank: 2♥-2♣",
    );
    gameLogger.info(
      "test_non_trump",
      { pair: "K♦-K♦" },
      "- Bot1 has non-trump: K♦-K♦",
    );
    gameLogger.info(
      "test_expectation",
      { expected: "use weakest trump (3♠-3♠)" },
      "Expected: Bot1 should use weakest trump (3♠-3♠)",
    );
    ...
    const usedWeakTrump = aiMove.every(
      (card) => card.suit === Suit.Spades && card.rank === Rank.Three,
    );
    expect(usedWeakTrump).toBe(true); // Should use 3♠-3♠

    // Should NOT use stronger trump
    const usedStrongTrump = aiMove.some(
      (card) =>
    ...
    const usedNonTrump = aiMove.some(
      (card) => card.suit === Suit.Diamonds && card.rank === Rank.King,
    );
  ...
  it("should handle edge case: no trump pairs available but has trump singles", () => {
    ...
    gameLogger.info(
      "test_trump_singles",
      { singles: ["3♠", "4♠", "2♣"] },
      "- Bot1 has trump singles: 3♠, 4♠, 2♣ (cannot form pairs)",
    );
    gameLogger.info(
      "test_non_trump_pairs",
      { pairs: ["K♥-K♥", "Q♦-Q♦"] },
      "- Bot1 has non-trump pairs: K♥-K♥, Q♦-Q♦",
    );
    gameLogger.info(
      "test_expectation",
      { expected: "use non-trump pair (no trump pairs available)" },
      "Expected: Should be forced to use non-trump pair (no trump pairs available)",
    );
    ...
    const usedNonTrump = aiMove.every(
      (card) =>
        card.suit !== trumpInfo.trumpSuit && card.rank !== trumpInfo.trumpRank,
    );
  ...
  it("should reproduce the specific wasteful scenario: trump pairs available but AI chooses non-trump", () => {
    ...
    gameLogger.info(
      "test_trump_pairs",
      { pairs: ["4♠-4♠", "2♥-2♣"] },
      "- Bot1 has trump pairs: 4♠-4♠, 2♥-2♣",
    );
    gameLogger.info(
      "test_attractive_non_trump",
      { pairs: ["A♥-A♥", "K♦-K♦"] },
      "- Bot1 has attractive non-trump pairs: A♥-A♥, K♦-K♦",
    );
    gameLogger.info(
      "test_trick_value",
      {
  ...
  it("should handle no trump cards available: must use non-trump pairs optimally", () => {
    ...
    gameLogger.info(
      "test_no_trump",
      { trumpCards: "none" },
      "- Bot1 has NO trump cards",
    );
    gameLogger.info(
      "test_available_cards",
      {
        nonTrumpPair: "7♥-7♥",
        singles: ["9♣", "J♣"],
      },
    ...
    gameLogger.info(
      "test_question",
      { question: "What should AI play?" },
      "Question: What should AI play?",
    );
    gameLogger.info(
      "test_option_a",
      { option: "7♥-7♥" },
      "Option A: 7♥-7♥ (use the pair)",
    );
    gameLogger.info(
      "test_option_b",
      { option: "9♣-J♣" },
      "Option B: 9♣-J♣ (some other combination)",
    );
    ...
    const usedHeartsPair = aiMove.every(
      (card) => card.suit === Suit.Hearts && card.rank === Rank.Seven,
    );

    const usedClubsSingles = aiMove.every(
      (card) =>
    ...
    const noTrumpUsed = aiMove.every(
      (card) =>
        card.suit !== trumpInfo.trumpSuit && card.rank !== trumpInfo.trumpRank,
    );
