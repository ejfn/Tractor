...
describe("AI Non-Trump Tractor Following: Pair Priority", () => {
  ...
  it("should use ALL available pairs before singles when following non-trump tractor", () => {
    ...
    gameState.currentTrick = {
      plays: [{ playerId: PlayerId.Bot2, cards: leadingTractor }],
    ...
    gameLogger.info(
      "test_non_trump_tractor_setup",
      { testType: "pair_priority" },
      "=== AI Non-Trump Tractor Following Test ===",
    );
    gameLogger.info(
      "test_leading_cards",
      { cards: "3♦3♦-4♦4♦", type: "Diamond tractor" },
      "Leading: 3♦3♦-4♦4♦ (Diamond tractor)",
    );
    gameLogger.info(
      "test_bot_hand",
      { hand: "A♦A♦, 7♦7♦, 8♦, 6♦, 2♣" },
      "Bot3 hand: A♦A♦, 7♦7♦, 8♦, 6♦, 2♣",
    );
    gameLogger.info(
      "test_expected_outcome",
      { expected: "A♦A♦7♦7♦" },
      "Expected: Bot3 should use A♦A♦7♦7♦ (all available pairs)",
    );
    ...
    const usedSingles = aiMove.filter(
      (c) => c.rank === Rank.Eight || c.rank === Rank.Six,
    ).length;

    gameLogger.info(
      "test_pair_analysis",
      { acePairs, sevenPairs, usedSingles },
      `Ace pairs used: ${acePairs}, Seven pairs used: ${sevenPairs}, Singles used: ${usedSingles}`,
    );
    ...
    gameLogger.info(
      "test_success",
      { result: "all_pairs_before_singles" },
      "✅ AI correctly uses ALL pairs before singles in non-trump tractor following",
    );
  });
  ...
  it("should use all pairs + singles when insufficient pairs for full tractor", () => {
    ...
    gameLogger.info(
      "test_insufficient_pairs_setup",
      { testType: "insufficient_pairs" },
      "\n=== Insufficient Pairs Scenario ===",
    );
    gameLogger.info(
      "test_bot_constraints",
      { availablePairs: 1, tractorSize: 4 },
      "Bot3 has only 1 pair A♦A♦ for 4-card tractor",
    );
    gameLogger.info(
      "test_expected_outcome",
      { expected: "A♦A♦ + 2 singles (8♦, 6♦)" },
      "Expected: A♦A♦ + 2 singles (8♦, 6♦)",
    );
    ...
    const diamondSingles = aiMove.filter(
      (c) => c.suit === Suit.Diamonds && c.rank !== Rank.Ace,
    ).length;
    expect(diamondSingles).toBe(2);

    gameLogger.info(
      "test_success",
      { result: "pair_plus_singles_when_insufficient" },
      "✅ AI correctly uses available pair + singles when insufficient pairs",
    );
  });
  ...
  it("should not use valuable pairs when opponent is winning (strategic disposal)", () => {
    ...
    gameLogger.info(
      "test_strategic_disposal_setup",
      { testType: "strategic_disposal" },
    ...
    gameLogger.info(
      "test_game_state",
      {
        ...
        valuablePairs: "A♦A♦, K♦K♦",
        lowerPairs: "7♦7♦, 8♦8♦",
      },
      "Opponent winning, Bot3 has valuable pairs (A♦A♦, K♦K♦) and lower pairs (7♦7♦, 8♦8♦)",
    );
    gameLogger.info(
      "test_expected_outcome",
      { expected: "7♦7♦, 8♦8♦" },
      "Expected: Use lower value pairs (7♦7♦, 8♦8♦) when opponent winning",
    );
    ...
    const valuablePairs = aiMove.filter(
      (c) => c.rank === Rank.Ace || c.rank === Rank.King,
    ).length;
    const lowPairs = aiMove.filter(
    ...
    gameLogger.info(
      "test_success",
      { result: "avoids_valuable_pairs_when_opponent_winning" },
      "✅ AI correctly avoids valuable pairs when opponent winning",
    );
  });
});
