...
describe("Attacking Leading Strategy Bug Test", () => {
  it("should NOT lead with trump suit high cards even in desperate attacking scenarios", () => {
    ...
    gameLogger.info(
      "test_attacking_leading_setup",
      {},
      "=== ATTACKING LEADING STRATEGY TEST ===",
    );
    gameLogger.info(
      "test_trump_suit_info",
      { trumpSuit: "Hearts" },
      "Trump suit: Hearts",
    );
    gameLogger.info(
      "test_ai_hand_info",
      {
        ...
        hand: aiBotHand.map(
          (c) =>
            `${c.rank}${c.suit === Suit.Hearts ? "♥" : c.suit === Suit.Spades ? "♠" : c.suit === Suit.Clubs ? "♣" : "♦"}`,
        ),
      },
      `AI Bot1 (attacking team): ${aiBotHand.map((c) => `${c.rank}${c.suit === Suit.Hearts ? "♥" : c.suit === Suit.Spades ? "♠" : c.suit === Suit.Clubs ? "♣" : "♦"}`).join(", ")}`,
    );
    gameLogger.info(
      "test_game_scenario",
      { gameStage: "late", opponentPoints: 100, scenario: "desperate" },
      "Game: Late game, 100 points to opponents (desperate scenario)",
    );
    gameLogger.info(
      "test_expected_strategy",
      { strategyType: "desperate_attacking", trumpPriority: "potential" },
      "Expected: Desperate attacking strategy with potential trump priority",
    );
    ...
    gameLogger.info(
      "test_ai_decision",
      { selectedCard: cardDisplay, playerId: "Bot1" },
      `AI selected: ${cardDisplay}`,
    );

    // Check if AI led with trump suit high card
    const ledTrumpSuitHighCard =
      selectedCard.suit === Suit.Hearts &&
      (selectedCard.rank === Rank.Ace ||
        selectedCard.rank === Rank.King ||
        selectedCard.rank === Rank.Ten);

    if (ledTrumpSuitHighCard) {
      gameLogger.info(
        "test_bug_detection",
        { bugType: "trump_suit_high_card_lead", confirmed: true },
        "❌ BUG CONFIRMED: Attacking strategy led with trump suit high card!",
      );
      gameLogger.info(
        "test_bug_analysis",
        {
          likelySource: "selectByStrength",
          preferTrump: true,
          mode: "desperate",
        },
        "This is likely from selectByStrength with preferTrump: true in desperate mode",
      );
    } else {
      gameLogger.info(
        ...
        {
          result: "good",
          strategyType: "desperate_attacking",
          avoidedTrumpHigh: true,
        },
        "✅ GOOD: Even desperate attacking strategy avoided trump suit high cards",
      );
    }
  ...
  it("should test the specific desperate + trump priority code path", () => {
    ...
    gameLogger.info(
      "test_desperate_trump_priority_setup",
      {},
    ...
    gameLogger.info(
      "test_ai_hand_info",
      {
        ...
        hand: aiBotHand.map(
          (c) =>
            `${c.rank}${c.suit === Suit.Diamonds ? "♦" : c.suit === Suit.Spades ? "♠" : "?"}`,
        ),
      },
      `AI hand: ${aiBotHand.map((c) => `${c.rank}${c.suit === Suit.Diamonds ? "♦" : c.suit === Suit.Spades ? "♠" : "?"}`).join(", ")}`,
    );
    gameLogger.info(
      ...
      "Conditions: Late game, attacking team losing badly (180 points to opponents)",
    );
    ...
    gameLogger.info(
      "test_ai_decision",
      { selectedCard: cardDisplay, playerId: "Bot2" },
      `AI selected: ${cardDisplay}`,
    );
    ...
    if (ledTrumpHigh) {
      gameLogger.info(
        ...
        {
          issueType: "trump_suit_high_card_lead",
          source: "selectByStrength",
          preferTrump: true,
        },
        "❌ ISSUE: selectByStrength with preferTrump:true led trump suit high card",
      );
      gameLogger.info(
        "test_issue_source",
        {
          playStyle: "Desperate",
          path: "trumpPriority",
          function: "selectAttackingLeadPlay",
        },
        "Source: PlayStyle.Desperate + trumpPriority path in selectAttackingLeadPlay",
      );
    } else {
      gameLogger.info(
        "test_protection_validation",
        { result: "protected", trumpPriority: true, avoidedTrumpHigh: true },
        "✅ Protected: Even with trump priority, avoided trump suit high cards",
      );
    }

    gameLogger.info(
      "test_final_result",
      { ledTrumpSuitHighCard: ledTrumpHigh },
      `Led trump suit high card: ${ledTrumpHigh}`,
    );
  });
});
