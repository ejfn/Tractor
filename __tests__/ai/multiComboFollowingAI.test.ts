...
describe("AI Multi-Combo Following Strategy", () => {
  ...
  describe("Same-Suit Structure Matching", () => {
    test("MCF-AI-1: AI should match structure with best strategic cards", () => {
      ...
      // Bot hand: Can form structure but has choices
      const playerHand = [
        Card.createCard(Suit.Spades, Rank.Ace, 0),    // High card
        Card.createCard(Suit.Spades, Rank.Ace, 1),    // High pair
      ...
      expect(result!.canBeat).toBe(true); // Aces should beat Kings
      
      // Should use Ace pair to beat King pair
    ...
    test("MCF-AI-2: AI should use conservative cards when can't beat", () => {
      ...
      const usedTenPair = result!.cards.filter(card => card.rank === Rank.Ten).length === 2;
      expect(usedTenPair).toBe(true); // Must use 10♠-10♠ pair (only pair available)
      
      // Should use lowest singles to complete the structure
      const usedLowSingles = result!.cards.filter(card => 
        card.rank === Rank.Seven || card.rank === Rank.Six
      ).length >= 2;
  ...
  describe("Trump Opportunity When Void", () => {
    ...
    test("MCF-AI-4: AI should use lowest trump when beating non-trump", () => {
      ...
      const usesLowTrump = result!.cards.every(card => 
        card.suit === Suit.Hearts && (card.rank === Rank.Three || card.rank === Rank.Four)
      );
  ...
  describe("Trump vs Trump Multi-Layer Comparison", () => {
    test("MCF-AI-5: AI should beat previous trump response with higher trump", () => {
      ...
      const hasAcePair = result!.cards.filter(card => card.rank === Rank.Ace).length === 2;
  ...
  describe("Strategic Disposal When Can't Beat", () => {
    test("MCF-AI-6: AI should dispose lowest value cards when can't beat", () => {
      ...
      const preservesValueCards = result!.cards.every(card => 
        card.rank !== Rank.Ten && card.rank !== Rank.King
      );
