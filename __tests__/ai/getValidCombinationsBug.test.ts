...
describe("getValidCombinations Bug Reproduction", () => {
  it("should NEVER return empty array - reproduce the exact bug scenario", () => {
    ...
    gameLogger.info(
      "test_function_call",
      {},
      "About to call getValidCombinations...",
    );
    const validCombinations = getValidCombinations(playerHand, gameState);
    gameLogger.info(
      "test_function_complete",
      {},
      "getValidCombinations completed",
    );
    ...
    if (validCombinations.length === 0) {
      gameLogger.error(
        "test_bug_confirmed",
        {},
        "🐛 BUG CONFIRMED: getValidCombinations returned empty array!",
      );
      gameLogger.error(
        "test_bug_explanation",
        {},
        "This should NEVER happen in Tractor - there should always be a valid play",
      );
    } else {
      gameLogger.info(
    ...
    const foundValidCombo = validCombinations.some(
      (combo) =>
