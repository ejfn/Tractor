...
describe("Memory Performance Integration Tests - Phase 4", () => {
  ...
  describe("Memory Caching Performance", () => {
    ...
    it("should invalidate cache when game state changes", () => {
      ...
      expect(memory1.playedCards.length).toBeLessThan(
        memory2.playedCards.length,
      );
    });
  ...
  describe("Memory vs Performance Trade-offs", () => {
    it("should provide equivalent results between optimized and standard memory", () => {
      ...
      expect(optimizedMemory.playedCards.length).toBe(
        standardMemory.playedCards.length,
      );
      expect(optimizedMemory.tricksAnalyzed).toBe(
        standardMemory.tricksAnalyzed,
      );
      expect(optimizedMemory.trumpCardsPlayed).toBe(
        standardMemory.trumpCardsPlayed,
      );
      expect(optimizedMemory.pointCardsPlayed).toBe(
        standardMemory.pointCardsPlayed,
      );
      ...
      Object.keys(optimizedMemory.playerMemories).forEach((playerId) => {
        ...
        expect(optimizedPlayer.suitVoids.size).toBe(
          standardPlayer.suitVoids.size,
        );
        expect(optimizedPlayer.knownCards.length).toBe(
          standardPlayer.knownCards.length,
        );
      });
      ...
      gameLogger.info("equivalence_validation", {
        ...
        playersWithVoids: Object.values(optimizedMemory.playerMemories).filter(
          (p) => p.suitVoids.size > 0,
        ).length,
      });
    });
