...
describe('Human Multi-Card Loss Bug', () => {
  test('Reproduce human losing multiple cards', () => {
    ...
    if (humanLost > humanCardsToPlay.length) {
      ...
      const beforeIds = beforeState.players[humanIndex].hand.map(c => c.id);
      const afterIds = result.newState.players[humanIndex].hand.map(c => c.id);
      const lostIds = beforeIds.filter(id => !afterIds.includes(id));
      
      console.log(`Lost card IDs: ${lostIds.join(', ')}`);
      console.log(`Expected to lose: ${humanCardsToPlay.map(c => c.id).join(', ')}`);
      
      const unexpectedLoss = lostIds.filter(id => !humanCardsToPlay.some(c => c.id === id));
