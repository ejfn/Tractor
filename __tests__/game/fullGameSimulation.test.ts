...
describe("Full Game Simulation", () => {
  test("Play complete game monitoring all card counts", () => {
    ...
    while (state.players.every((p) => p.hand.length > 0) && totalErrors < 3) {
      ...
      for (
        let trickNum = 0;
        trickNum < 5 && state.players.every((p) => p.hand.length > 0);
        trickNum++
      ) {
        gameLogger.info(
          "test_trick_start",
        ...
        gameLogger.info(
          "test_trick_card_counts",
          { counts: trickStartCounts },
          `Starting counts: ${trickStartCounts.join(", ")}`,
        );
        ...
        for (let playNum = 0; playNum < 4; playNum++) {
          ...
          if (cardsToPlay.length === 0) {
            gameLogger.error(
              ...
              `ERROR: ${currentPlayer.id} has no cards to play!`,
            );
            totalErrors++;
            break;
          }
          ...
          for (let i = 0; i < 4; i++) {
            ...
            const expected =
              i === state.currentPlayerIndex
                ? before - cardsToPlay.length
                : before;

            if (after !== expected) {
              anomalies.push(
                `Player ${i} (${state.players[i].id}): expected ${expected}, got ${after}`,
              );
            }
          }
          ...
          if (uniqueCounts.size > 1 && playNum === 3) {
            ...
            anomalies.push(
              `Unequal counts after trick: ${cardsAfter.join(", ")}`,
            );
          }

          if (anomalies.length > 0) {
            gameLogger.error(
              "test_card_count_anomalies",
              { round, trick: trickNum + 1, play: playNum + 1, anomalies },
              `\nANOMALIES in Round ${round}, Trick ${trickNum + 1}, Play ${playNum + 1}:`,
            );
            anomalies.forEach((a) =>
              gameLogger.error("test_anomaly_detail", { anomaly: a }, `  ${a}`),
            );
            totalErrors++;
          }
        ...
        const wrongCounts = trickEndCounts.filter(
          (c, i) => c !== expectedCount,
        );
        if (wrongCounts.length > 0) {
          gameLogger.error(
            "test_trick_end_error",
            {
              trick: trickNum + 1,
              expectedCount,
              actualCounts: trickEndCounts,
            },
            `ERROR: After trick ${trickNum + 1}, not all players have ${expectedCount} cards`,
          );
          gameLogger.error(
            "test_trick_end_counts",
            { counts: trickEndCounts },
        ...
        gameLogger.info(
          "test_trick_end",
          { winner: trickWinner, counts: trickEndCounts },
          `Trick winner: ${trickWinner}, End counts: ${trickEndCounts.join(", ")}`,
        );
      }
    ...
    if (firstDivergence) {
      ...
      gameLogger.error(
        ...
        { firstDivergence },
        `  Round ${firstDivergence.round}, Trick ${firstDivergence.trick + 1}, Play ${firstDivergence.play + 1}`,
      );
      gameLogger.error(
        "test_divergence_player",
        { player: firstDivergence.player },
        `  Player: ${firstDivergence.player}`,
      );
      gameLogger.error(
        "test_divergence_before",
        { cardsBefore: firstDivergence.cardsBefore },
        `  Cards before: ${firstDivergence.cardsBefore.join(", ")}`,
      );
      gameLogger.error(
        "test_divergence_after",
        { cardsAfter: firstDivergence.cardsAfter },
        `  Cards after: ${firstDivergence.cardsAfter.join(", ")}`,
      );
      ...
      const contextEnd = Math.min(
        gameLog.length,
        gameLog.indexOf(firstDivergence) + 4,
      );
      ...
      for (let i = contextStart; i < contextEnd; i++) {
        ...
        const marker =
          i === gameLog.indexOf(firstDivergence) ? " <-- DIVERGENCE" : "";
        gameLogger.error(
          "test_divergence_context_entry",
          { entry, marker },
          `  ${entry.player}: ${entry.cardsBefore.join(",")} -> ${entry.cardsAfter.join(",")}${marker}`,
        );
      }
    }
    ...
    players.forEach((playerName, idx) => {
      ...
      const totalCardsPlayed = playerEntries.reduce(
        (sum, e) => sum + e.cardsPlayed,
        0,
      );
      ...
      gameLogger.info(
        "test_player_plays",
        { playerName, totalPlays: playerEntries.length },
        `  Total plays: ${playerEntries.length}`,
      );
      gameLogger.info(
        "test_player_cards",
        { playerName, totalCardsPlayed },
      ...
      if (anomalous.length > 0) {
        gameLogger.error(
          "test_player_anomalies",
          { playerName, anomalousCount: anomalous.length },
          `  ANOMALIES: ${anomalous.length} plays with incorrect card removal`,
        );
        anomalous.slice(0, 3).forEach((a) => {
          gameLogger.error(
            "test_anomaly_example",
            {
              playerName,
              round: a.round,
              trick: a.trick + 1,
