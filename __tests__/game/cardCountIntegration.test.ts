...
describe('Card Count Integration Test', () => {
  test('Track card removal through processPlay', () => {
    ...
    for (let play = 0; play < 4; play++) {
      ...
      gameLogger.info('test_playing_cards', { cardCount: cardsToPlay.length, cardInfo: `${cardsToPlay[0].suit || 'JOKER'}${cardsToPlay[0].rank || ''}` }, `Playing ${cardsToPlay.length} card(s): ${cardsToPlay[0].suit || 'JOKER'}${cardsToPlay[0].rank || ''}`);
      ...
      gameLogger.info('test_card_counts_after', { cardCounts: state.players.map(p => p.hand.length) }, `Card counts after: ${state.players.map(p => p.hand.length).join(', ')}`);
      gameLogger.info('test_new_current_player', { currentPlayerIndex: state.currentPlayerIndex }, `New current player: ${state.currentPlayerIndex}`);
    ...
    gameLogger.info('test_final_card_counts', { finalCounts }, `\nFinal card counts: ${finalCounts.join(', ')}`);
  ...
  test('Track processPlay with state mutations', () => {
    ...
    // Make first play
    gameLogger.info('test_state_mutation_start', {}, '\n=== Testing state mutation ===');
    const originalState = gameState;
    const player0Cards = originalState.players[0].hand.map(c => c.id);
    
    gameLogger.info('test_player_0_initial_cards', { cardCount: originalState.players[0].hand.length }, `Player 0 has ${originalState.players[0].hand.length} cards`);
    
    // Process human play
    const result1 = processPlay(originalState, [originalState.players[0].hand[0]]);
    
    gameLogger.info('test_after_play', {}, `After play:`);
    gameLogger.info('test_original_state_cards', { cardCount: originalState.players[0].hand.length }, `  Original state player 0: ${originalState.players[0].hand.length} cards`);
    gameLogger.info('test_result_state_cards', { cardCount: result1.newState.players[0].hand.length }, `  Result state player 0: ${result1.newState.players[0].hand.length} cards`);
    ...
    if (player0Cards.length !== player0CardsAfter.length) {
      gameLogger.error('test_original_state_mutated', {}, 'ERROR: Original state was mutated!');
      gameLogger.error('test_mutation_details', { before: player0Cards.length, after: player0CardsAfter.length }, `  Before: ${player0Cards.length} cards`);
      gameLogger.error('test_mutation_details_after', { after: player0CardsAfter.length }, `  After: ${player0CardsAfter.length} cards`);
    }
    ...
    gameLogger.info('test_card_counts_before_2', { cardCounts: cardsBefore }, `  Before: ${cardsBefore.join(', ')}`);
    gameLogger.info('test_card_counts_after_2', { cardCounts: result2.newState.players.map(p => p.hand.length) }, `  After: ${result2.newState.players.map(p => p.hand.length).join(', ')}`);
