...
import { createTrumpScenarios, testData } from "../helpers";
...
describe("Combo Type Identification Tests", () => {
  ...
  describe("Tractor Combos", () => {
    ...
    test("Non-consecutive pairs should not form a tractor", () => {
      ...
      expect(getComboType(nonConsecutivePairs, trumpInfo)).not.toBe(
        ComboType.Tractor,
      );
    });
    ...
    test("Consecutive pairs of different suits should not form a tractor", () => {
      ...
      // Should not be identified as a tractor
      expect(getComboType(mixedSuitPairs, trumpInfo)).not.toBe(
        ComboType.Tractor,
      );
    });
    ...
    test("Trump tractors should beat non-trump tractors", () => {
      ...
      expect(getComboType(trumpTractor, trumpInfoHearts)).toBe(
        ComboType.Tractor,
      );
      expect(getComboType(nonTrumpTractor, trumpInfoHearts)).toBe(
        ComboType.Tractor,
      );

      // Compare individual cards to verify that trump beats non-trump
      expect(
        compareCards(heart3pair[0], spadeQpair[0], trumpInfoHearts),
      ).toBeGreaterThan(0);
      expect(
        compareCards(heart4pair[0], spadeKpair[0], trumpInfoHearts),
      ).toBeGreaterThan(0);
    });
  });
