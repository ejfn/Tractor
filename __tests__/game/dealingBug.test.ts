...
describe("Card Dealing Bug Test", () => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      gameState.players.forEach((player, idx) => {
        gameLogger.info(
          ...
          `  Player ${idx} (${player.id}): ${player.hand.length} cards`,
        );
      });
      ...
      if (uniqueCounts.size > 1) {
        gameLogger.error(
          "test_card_count_mismatch",
          { cardCounts },
          "ERROR: Players have different initial card counts!",
        );
        gameLogger.error(
          "test_card_count_details",
          { cardCounts },
          `Counts: ${cardCounts.join(", ")}`,
        );
      }
  ...
  test("Check the deck distribution math", () => {
    ...
    gameLogger.info(
      ...
      { cardsForPlayers, cardsPerPlayer, remainder },
      `Cards for players: ${cardsForPlayers}`,
    );
    gameLogger.info(
      "test_cards_per_player",
      { cardsPerPlayer },
      `Cards per player (floor): ${cardsPerPlayer}`,
    );
    ...
    gameLogger.info(
      "test_cards_distributed",
      { playerCards },
      `Cards distributed: ${playerCards.join(", ")}`,
    );
    gameLogger.info(
      "test_total_distributed",
      { totalDistributed: playerCards.reduce((sum, n) => sum + n, 0) },
      `Total distributed: ${playerCards.reduce((sum, n) => sum + n, 0)}`,
    );
    ...
    gameLogger.info(
      "test_last_player_indices",
      { lastPlayerStartIdx, lastPlayerEndIdx },
      `Last player (Bot 3) indices: ${lastPlayerStartIdx} to ${lastPlayerEndIdx}`,
    );
    gameLogger.info(
      "test_deck_minus_kitty",
      { deckSizeMinusKitty: deck.length - kittySize },
      `Deck length minus kitty: ${deck.length - kittySize}`,
    );

    if (lastPlayerEndIdx > cardsForPlayers) {
      gameLogger.error(
        "test_index_overflow",
        { lastPlayerEndIdx, cardsForPlayers },
        `ERROR: Last player end index (${lastPlayerEndIdx}) exceeds available cards (${cardsForPlayers})`,
      );
    }
  });
  ...
  test("Simulate the exact dealing logic", () => {
    ...
    const cardsPerPlayer = Math.floor(
      (shuffledDeck.length - 8) / players.length,
    );

    gameLogger.info(
      "test_dealing_simulation",
      { cardsPerPlayer, deckSize: shuffledDeck.length },
    ...
    players.forEach((player, index) => {
      ...
      gameLogger.info(
        "test_player_dealing",
        {
          ...
          startIdx,
          endIdx,
          cardsReceived: player.hand.length,
        },
        `Player ${index} (${player.name}): indices ${startIdx}-${endIdx}, got ${player.hand.length} cards`,
      );
    });
    ...
    gameLogger.info(
      "test_kitty_dealing",
      { kittyStart, deckLength: shuffledDeck.length, kittySize: kitty.length },
      `Kitty: indices ${kittyStart}-${shuffledDeck.length}, got ${kitty.length} cards`,
    );

    // Verify all cards are accounted for
    const totalCardsDealt =
      players.reduce((sum, p) => sum + p.hand.length, 0) + kitty.length;
    gameLogger.info(
      "test_total_cards_verification",
      { totalCardsDealt, expectedTotal: shuffledDeck.length },
    ...
    if (allIndices.size !== shuffledDeck.length) {
      gameLogger.error(
        "test_index_verification_failure",
        { uniqueIndicesUsed: allIndices.size, totalCards: shuffledDeck.length },
        `ERROR: Only ${allIndices.size} unique indices used out of ${shuffledDeck.length} cards`,
      );
      ...
      for (let i = 0; i < shuffledDeck.length; i++) {
        if (!allIndices.has(i)) {
          gameLogger.error(
            "test_missed_card",
            { cardIndex: i },
            `  Card at index ${i} was not dealt to anyone!`,
          );
        }
      }
    }
