  ...
  test("Simulate race condition during trick completion", async () => {
    ...
    for (let play = 0; play < 4; play++) {
      ...
      if (result.trickComplete) {
        ...
        gameLogger.info(
          "test_next_trick_start",
          { playerName: nextPlayer.id },
          `Next trick starts with ${nextPlayer.id}`,
        );
      }
    }
  ...
  test("Check for state mutations during async operations", async () => {
    ...
    for (let play = 0; play < 4; play++) {
      ...
      if (result.trickComplete) {
        ...
        const currentCardCounts = originalState.players.map(
          (p) => p.hand.length,
        );

        if (
          JSON.stringify(originalCardCounts) !==
          JSON.stringify(currentCardCounts)
        ) {
          gameLogger.error(
            "test_state_mutation_detected",
            {},
            "ERROR: Original state was mutated during async operation!",
          );
          gameLogger.error(
            "test_original_card_counts",
            { originalCardCounts },
            `  Original counts: ${originalCardCounts.join(",")}`,
          );
          gameLogger.error(
            "test_current_card_counts",
            { currentCardCounts },
            `  Current counts: ${currentCardCounts.join(",")}`,
          );
        }

        if (originalState.currentPlayerIndex !== originalPlayerIndex) {
          gameLogger.error(
            "test_player_index_mutation",
            {},
            "ERROR: Original currentPlayerIndex was mutated!",
          );
        }
      } else {
        state = result.newState;
