...
describe("Cross-Suit Pair Bug Fix", () => {
  test("should NOT form pairs between different suit trump rank cards", () => {
    ...
    gameLogger.info(
      "test_cross_suit_cards",
      { cards: cards.map((c) => `${c.rank}${c.suit} (cardId: ${c.commonId})`) },
      "Testing cards: " +
        cards
          .map((c) => `${c.rank}${c.suit} (cardId: ${c.commonId})`)
          .join(", "),
    );
    ...
    if (pairs.length > 0) {
      pairs.forEach((pair) => {
        gameLogger.info(
          "test_pair_details",
          {
            pairCards: pair.cards.map(
              (c) => `${c.rank}${c.suit} (${c.commonId})`,
            ),
          },
          "Pair: " +
            pair.cards
              .map((c) => `${c.rank}${c.suit} (${c.commonId})`)
              .join(", "),
        );
      });
    }
  ...
  test("should form pairs between identical cards from different decks", () => {
    ...
    gameLogger.info(
      "test_identical_cards",
      {
        cards: cards.map(
          (c) =>
            `${c.rank}${c.suit} (cardId: ${c.commonId}, deckId: ${c.deckId})`,
        ),
      },
  ...
  test("should handle complex cross-suit trump scenarios correctly", () => {
    ...
    const crossSuitPair = pairs.find(
      (p) => p.cards[0].suit !== p.cards[1].suit,
    );
