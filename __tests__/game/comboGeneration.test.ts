...
describe("Combo Generation Comprehensive Tests", () => {
  ...
  describe("Valid Combo Identification", () => {
    test("should identify singles, pairs, and tractors correctly", () => {
      ...
      const longestTractor = tractors.reduce(
        (longest, current) =>
    ...
    test("should handle trump rank cards correctly", () => {
      ...
      const heartsPair = pairs.find((p) =>
        p.cards.every(
          (card) => card.rank === Rank.Two && card.suit === Suit.Hearts,
        ),
      );
  ...
  describe("Mixed Combo Generation", () => {
    test("should generate trump conservation combos when following trump pairs", () => {
      ...
      gameState.currentTrick = {
        plays: [{ playerId: PlayerId.Bot1, cards: leadingTrumpPair }],
      ...
      gameLogger.info(
        "test_trump_conservation_values",
        { usedConservationValues },
        "Trump conservation combo values: " +
          JSON.stringify(usedConservationValues),
      );
      ...
      const totalConservationValue = usedConservationValues.reduce(
        (sum, val) => sum + val,
        0,
      );
      gameLogger.info(
        "test_total_conservation_value",
        { totalConservationValue },
        "Total conservation value: " + totalConservationValue,
      );
    ...
    test("should handle partial suit following correctly", () => {
      ...
      gameState.currentTrick = {
        plays: [{ playerId: PlayerId.Bot1, cards: leadingHeartsPair }],
        winningPlayerId: PlayerId.Bot1,
        points: 0,
      };
      ...
      validCombos.forEach((combo) => {
        const hasHeartCard = combo.cards.some(
      ...
      const heartCards = optimalCombo.cards.filter(
        (card) => card.suit === Suit.Hearts,
      );
    ...
    test("should generate valid combos when out of led suit", () => {
      ...
      gameState.currentTrick = {
        plays: [{ playerId: PlayerId.Bot1, cards: leadingHearts }],
        winningPlayerId: PlayerId.Bot1,
        points: 0,
      };
  ...
  describe("Trump Following Edge Cases", () => {
    test("should handle mixed trump combinations when cannot form proper trump pairs", () => {
      ...
      const usedBigJoker = optimalCombo.cards.some(
        (card) => card.joker === JokerType.Big,
      );
      const usedTrumpSuit = optimalCombo.cards.some(
      ...
      if (usedTrumpSuit) {
        gameLogger.info(
          "test_trump_prioritization",
          { usedTrumpSuit, usedBigJoker },
          "Correctly prioritized trump suit card over Big Joker",
        );
      }
    });
    ...
    test("should handle joker tractor following correctly", () => {
      ...
      gameState.currentTrick = {
        plays: [{ playerId: PlayerId.Bot1, cards: leadingJokerTractor }],
        winningPlayerId: PlayerId.Bot1,
        points: 0,
      };
      ...
      const jokerTractorCombo = validCombos.find(
        (combo) =>
          combo.cards.length === 4 &&
          combo.cards.filter((card) => card.joker === JokerType.Small)
            .length === 2 &&
          combo.cards.filter((card) => card.joker === JokerType.Big).length ===
            2,
      );

      expect(jokerTractorCombo).toBeDefined();
      if (!jokerTractorCombo) return;
  ...
  describe("Strategic Value Calculation", () => {
    test("should calculate trump conservation values correctly", () => {
      ...
      gameLogger.info(
        "test_trump_hierarchy_values",
        { values },
        "Trump hierarchy values: " + JSON.stringify(values),
      );
      ...
      const nonTrumpStrategic = calculateCardStrategicValue(
        nonTrumpCard,
        trumpInfo,
        "strategic",
      );
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    test("should always return at least one valid combo for non-empty hands", () => {
      const playerHand: Card[] = [Card.createCard(Suit.Clubs, Rank.Seven, 0)];
      ...
      gameState.currentTrick = {
        plays: [{ playerId: PlayerId.Bot1, cards: leadingCombo }],
        winningPlayerId: PlayerId.Bot1,
        points: 0,
      };
    ...
    test("should handle complex tractor requirements", () => {
      ...
      gameState.currentTrick = {
        plays: [{ playerId: PlayerId.Bot1, cards: leadingTractor }],
        winningPlayerId: PlayerId.Bot1,
        points: 5,
      };
      ...
      if (comboWithPair) {
        gameLogger.info(
          "test_pair_usage_in_tractor",
          { comboWithPair: comboWithPair.cards.length },
          "Correctly used available pair in tractor following",
        );
      }
    });
  });
  ...
  describe("Integration with AI Decision Making", () => {
    test("should provide multiple strategic options for AI choice", () => {
      ...
      for (let i = 1; i < validCombos.length; i++) {
        const prevValue = validCombos[i - 1].cards.reduce(
          (sum, card) =>
            sum + calculateCardStrategicValue(card, trumpInfo, "strategic"),
          0,
        );
        const currentValue = validCombos[i].cards.reduce(
          (sum, card) =>
            sum + calculateCardStrategicValue(card, trumpInfo, "strategic"),
          0,
        );
