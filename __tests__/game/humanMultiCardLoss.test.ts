...
describe('Human Multi-Card Loss Bug', () => {
  test('Reproduce human losing multiple cards', () => {
    ...
    for (let play = 0; play < 4; play++) {
      ...
      gameLogger.info('test_player_card_change', { playerName: currentPlayer.id, before: cardsBefore[state.currentPlayerIndex], after: cardsAfter[state.currentPlayerIndex] }, `${currentPlayer.id}: ${cardsBefore[state.currentPlayerIndex]} -> ${cardsAfter[state.currentPlayerIndex]}`);
    ...
    gameLogger.info('test_first_player', { playerName: firstPlayer.id, playerIndex: state.currentPlayerIndex }, `\nFirst player: ${firstPlayer.id} (index ${state.currentPlayerIndex})`);
    ...
    for (let i = 0; i < 4; i++) {
      ...
      if (diff !== 0) {
        gameLogger.info('test_player_lost_cards', { playerIndex: i, playerName: state.players[i].id, cardsLost: diff }, `Player ${i} (${state.players[i].id}): lost ${diff} cards`);
    ...
    for (let play = 1; play < 4; play++) {
      ...
      gameLogger.info('test_player_card_change_trick2', { playerName: currentPlayer.id, before: cardsBefore[state.currentPlayerIndex], after: cardsAfter[state.currentPlayerIndex] }, `${currentPlayer.id}: ${cardsBefore[state.currentPlayerIndex]} -> ${cardsAfter[state.currentPlayerIndex]}`);
      
      // Check for anomalies
      for (let i = 0; i < 4; i++) {
        const diff = cardsBefore[i] - cardsAfter[i];
        if (diff > 0 && i !== state.currentPlayerIndex) {
          gameLogger.error('test_non_playing_player_lost_cards', { playerIndex: i, playerName: state.players[i].id, cardsLost: diff }, `ERROR: Player ${i} (${state.players[i].id}) lost ${diff} cards but wasn't playing`);
        }
      }
    ...
    finalCounts.forEach((count, idx) => {
      if (count !== expectedCount) {
        gameLogger.error('test_final_card_count_error', { playerIndex: idx, playerName: state.players[idx].id, actualCount: count, expectedCount }, `ERROR: Player ${idx} (${state.players[idx].id}) has ${count} cards, expected ${expectedCount}`);
      }
    });
  });
});