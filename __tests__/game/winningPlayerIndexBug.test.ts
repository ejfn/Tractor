  ...
  test("should correctly determine trick winner and set current player", () => {
    ...
    for (let play = 0; play < 4; play++) {
      ...
      if (result.trickComplete) {
        ...
        const expectedWinnerIndex = result.newState.players.findIndex(
          (p) => p.id === result.trickWinnerId,
        );
        ...
        const winnerPlayer = result.newState.players.find(
          (p) => p.id === result.newState.currentTrick?.winningPlayerId,
        );
    ...
    for (let trickNum = 0; trickNum < 2; trickNum++) {
      for (let play = 0; play < 4; play++) {
        ...
        if (result.trickComplete) {
          ...
          const winnerById = result.newState.players.find(
            (p) => p.id === result.trickWinnerId,
          );
          const winnerByCurrentTrick = result.newState.players.find(
            (p) => p.id === result.newState.currentTrick?.winningPlayerId,
          );
