...
describe("AI Reasoning Bug Fix", () => {
  ...
  test("should generate correct reasoning messages with proper structure", () => {
    ...
    gameLogger.info(
      "test_bot2_decision",
      {
        shouldDeclare: bot2Decision.shouldDeclare,
        reasoning: bot2Decision.reasoning,
        decision: bot2Decision,
      },
      "Bot2 decision:",
    );
    ...
    if (bot2Decision.shouldDeclare) {
      expect(bot2Decision.reasoning).toContain(
        "Overriding human's pair with big joker pair",
      );
  ...
  test("should format declaration types properly in reasoning", () => {
    ...
    gameLogger.info(
      "test_bot1_initial_decision",
      {
        shouldDeclare: bot1Decision.shouldDeclare,
        reasoning: bot1Decision.reasoning,
        decision: bot1Decision,
      },
  ...
  test("should handle cases where declarer information might be missing", () => {
    ...
    if (bot1Decision.shouldDeclare) {
      expect(bot1Decision.reasoning).toMatch(
        /Declaring .+ early \(\d+% dealt\) to establish trump/,
      );
    } else {
      expect(bot1Decision.reasoning).toMatch(
        /Waiting for better opportunity - only .+ available/,
      );
    }
  });
  ...
  test("should demonstrate the bug fix: proper object structure access", () => {
    ...
    const stateWithDeclaration = makeTrumpDeclaration(
      gameState,
      PlayerId.Human,
      humanDeclaration,
    );
    ...
    gameLogger.info(
      ...
      { reasoning: bot2Decision.reasoning },
      "✅ Fixed reasoning: " + bot2Decision.reasoning,
    );
  ...
  test("should demonstrate forced override scenario with proper reasoning", () => {
    ...
    const stateAfterHuman = makeTrumpDeclaration(
      gameState,
      PlayerId.Human,
      humanDeclaration,
    );
    ...
    try {
      ...
      const stateAfterBot2 = makeTrumpDeclaration(
        stateAfterHuman,
        PlayerId.Bot2,
        bot2Declaration,
      );
      ...
      gameLogger.info(
        "test_bot2_override_success",
        {},
        "✅ Bot2 successfully overrode human declaration",
      );
      gameLogger.info(
        "test_final_trump_suit",
        { trumpSuit: stateAfterBot2.trumpInfo.trumpSuit },
        "Final trump suit: " + stateAfterBot2.trumpInfo.trumpSuit,
      );
      ...
      expect(
        stateAfterBot2.trumpDeclarationState?.currentDeclaration?.playerId,
      ).toBe(PlayerId.Bot2);
    } catch (error) {
      gameLogger.info(
        "test_declaration_failed",
