...
import { Card, ComboType, JokerType, Rank, Suit } from "../../src/types";
...
const [aceHearts1] = Card.createPair(Suit.Hearts, Rank.Ace);
...
describe("Joker Pair Tests", () => {
  ...
  test("SJ-SJ-BJ-BJ is recognized as a Tractor", () => {
    ...
    const result = getComboType(
      [smallJoker1, smallJoker2, bigJoker1, bigJoker2],
      trumpInfo,
    );
  ...
  test("identifyCombos finds Small Joker pairs", () => {
    ...
    const hasSmallJokerPair = combos.some(
      (combo) =>
  ...
  test("identifyCombos finds Big Joker pairs", () => {
    ...
    // Check that the pair is identified
    const hasBigJokerPair = combos.some(
      (combo) =>
        combo.type === ComboType.Pair &&
        combo.cards.length === 2 &&
  ...
  test("identifyCombos finds special SJ-SJ-BJ-BJ tractor", () => {
    ...
    const hasSpecialTractor = combos.some(
      (combo) =>
        combo.type === ComboType.Tractor &&
        combo.cards.length === 4 &&
        combo.cards.filter((card) => card.joker === JokerType.Small).length ===
          2 &&
