...
import { createTestCardsGameState, testData } from "../helpers";
...
describe("playProcessing", () => {
  ...
  describe("processPlay", () => {
    test("should process a play and update the game state correctly", () => {
      ...
      expect(result.newState.currentTrick?.plays[0].playerId).toBe(
        PlayerId.Human,
      );
  ...
  describe("validatePlay", () => {
    ...
    describe("when following a trick", () => {
      beforeEach(() => {
        ...
        (
          comboDetection.checkSameSuitPairPreservation as jest.Mock
        ).mockReturnValue(true);
        // Mock checkTractorFollowingPriority to return true
        (
          comboDetection.checkTractorFollowingPriority as jest.Mock
        ).mockReturnValue(true);
      });
    ...
    describe("edge cases", () => {
      test("should return false for invalid plays", () => {
        ...
        expect(
          validatePlay(null as unknown as GameState, [
  ...
  describe("getAIMoveWithErrorHandling", () => {
    ...
    describe("when successful", () => {
      ...
      test("should return AI move when successful", () => {
        ...
        expect(aiLogic.getAIMove).toHaveBeenCalledWith(
          mockState,
          PlayerId.Bot1,
        );
  ...
  describe("State Immutability", () => {
    ...
    test("should not mutate the original game state when processing plays", () => {
      ...
      expect(originalState.players[0].hand.map((c) => c.id)).toEqual(
        originalPlayerHandIds,
      );

      // Verify new state has the card removed
      expect(result.newState.players[0].hand.length).toBe(1);
      expect(
        result.newState.players[0].hand.map((c) => c.id),
      ).not.toContainEqual(cardsToPlay[0].id);
    });
          ...
          for (let playNum = 0; playNum < 4; playNum++) {
            ...
            if (result.trickComplete) {
              ...
              const expectedTotalCards =
                initialCardCount * 4 - (trickNum * 4 + (playNum + 1));
  ...
  describe("Trick Winner Determination", () => {
    ...
    describe("Basic Rank Comparisons", () => {
      beforeEach(() => {
        ...
        (cardComparison.evaluateTrickPlay as jest.Mock).mockImplementation(
          (cards, trick, trumpInfo, hand) => {
            ...
            return {
              canBeat,
              isLegal: true,
              strength: playedValue,
              reason: `${playedCard.rank} vs ${leadingCard.rank}`,
            };
          },
        );
      });
      ...
      test("Should correctly update trick winner using evaluateTrickPlay", () => {
        ...
        expect(result1.newState.currentTrick?.winningPlayerId).toBe(
          PlayerId.Human,
        );
        ...
        expect(result2.newState.currentTrick?.winningPlayerId).toBe(
          PlayerId.Bot1,
        );
      ...
      test("Should handle trump cards beating non-trump cards", () => {
        ...
        (cardComparison.evaluateTrickPlay as jest.Mock).mockImplementation(
          (cards, trick, trumpInfo, hand) => {
            ...
            if (
              playedCard.suit === trumpInfo?.trumpSuit &&
              leadingCard.suit !== trumpInfo?.trumpSuit
            ) {
              return {
                canBeat: true,
                isLegal: true,
                strength: 100,
                reason: "Trump beats non-trump",
              };
            }
            return {
              canBeat: false,
      ...
      test("Should handle cross-suit trump scenarios correctly", () => {
        ...
        (cardComparison.evaluateTrickPlay as jest.Mock).mockImplementation(
          (cards, trick, trumpInfo, hand) => {
            ...
            if (
              playedCard.suit === trumpInfo?.trumpSuit &&
              leadingCard.rank === trumpInfo?.trumpRank
            ) {
              return {
                canBeat: false,
                isLegal: true,
                strength: 50,
                reason: "Trump suit loses to trump rank",
              };
            }
            return {
              canBeat: false,
      ...
      test("Should maintain winner through subsequent plays", () => {
        ...
        expect(result.newState.currentTrick?.winningPlayerId).toBe(
          PlayerId.Human,
        );
