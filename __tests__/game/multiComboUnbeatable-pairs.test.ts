...
describe('Pair Cards - isComboUnbeatable Tests', () => {
  ...
  describe('Trump Rank Exclusion Tests', () => {
    test('Pair K♥K♥ becomes unbeatable when trump rank is A (no higher Hearts pairs)', () => {
      ...
      const trumpInfoAce = createTrumpScenarios.customTrump(Rank.Ace, Suit.Spades);
      const playedCards: Card[] = [];
      const ownHand: Card[] = [];

      const result = isComboUnbeatable(combo, Suit.Hearts, playedCards, ownHand, trumpInfoAce);
    ...
    test('Pair Q♥Q♥ becomes unbeatable when trump rank K excludes K♥ from Hearts', () => {
      ...
      const trumpInfoKing = createTrumpScenarios.customTrump(Rank.King, Suit.Spades);
      const playedCards: Card[] = [];
      const ownHand: Card[] = [];

      const result = isComboUnbeatable(combo, Suit.Hearts, playedCards, ownHand, trumpInfoKing);
  ...
  describe('Multiple Pair Scenarios - Strategic Implications', () => {
    test('Two different middle pairs - 9♥9♥ vs 7♥7♥ competition', () => {
      ...
      const result1 = isComboUnbeatable(combo1, Suit.Hearts, playedCards, ownHand, trumpInfo);
      const result2 = isComboUnbeatable(combo2, Suit.Hearts, playedCards, ownHand, trumpInfo);
    ...
    test('Multi-suit pair comparison - Hearts vs Diamonds', () => {
      ...
      const heartsResult = isComboUnbeatable(heartsCombo, Suit.Hearts, playedCards, ownHand, trumpInfo);
      const diamondsResult = isComboUnbeatable(diamondsCombo, Suit.Diamonds, playedCards, ownHand, trumpInfo);
  ...
  describe('Edge Cases', () => {
    test('Pair with trump suit (None) returns false (conservative)', () => {
      ...
      const result = isComboUnbeatable(combo, Suit.None, playedCards, ownHand, trumpInfo);
