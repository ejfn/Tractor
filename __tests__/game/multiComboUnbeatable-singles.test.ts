...
describe('Single Cards - isComboUnbeatable Tests', () => {
  ...
  const trumpInfoAceRank: TrumpInfo = { trumpRank: Rank.Ace, trumpSuit: Suit.Spades };
  ...
  describe('Single Cards - isComboUnbeatable', () => {
    ...
    test('Single K♥ is unbeatable when A is trump rank (K becomes highest)', () => {
      ...
      const result = isComboUnbeatable(combo, Suit.Hearts, playedCards, ownHand, trumpInfoAceRank);
      expect(result).toBe(true);
    });
  });
